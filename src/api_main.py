...
from permission_manager import get_user_consent, get_analysis_mode, get_advanced_options
...
def view_scans_via_api():
    """Fetches and displays a simple list of scan history from the API."""
    try:
        resp = requests.get(f"{API_URL}/scans")
        if resp.status_code != 200:
            print(_center_text("Failed to list scans."))
            return
        ...
        _print_banner("SCAN HISTORY (FROM API)")
