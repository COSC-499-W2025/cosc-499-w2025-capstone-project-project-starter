...
def get_project_by_id(project_id, user_name=None):
    ...
    Returns:
        dict: Project data with nested structure including 'project_info', or None if not found/access denied
    """
    # Get current user if user_name not provided
    if user_name is None:
        user_name = AuthManager.get_current_username()
    ...
    try:
        ...
        # Parse the 'metadata' column (where ProjectAnalyzer stored the JSON)
        metadata_json = row[4]
        analysis_data = {}
        if metadata_json:
        ...
        # Construct the Standard Project Dictionary
        ...
        # Populate project_info with database values
        project_data['project_info']['id'] = row[0]
        project_data['project_info']['filename'] = row[1]
        project_data['project_info']['filepath'] = row[2]
        project_data['project_info']['status'] = row[3]
        project_data['project_info']['created_at'] = row[5].isoformat() if row[5] else None
