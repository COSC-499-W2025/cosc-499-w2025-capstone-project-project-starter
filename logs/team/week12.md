# Team log (November 17 - 23, 2025)

## Github usernames

- @Canadadubstep -> Rylan Millar
- @alexbatke -> Alex Batke
- @ColePowrie -> Cole Powrie
- @liamsto -> Liam Storgaard
- @willtilden -> Will Tilden
- @LuisWenLuo -> Luis Wen

## Overview

- Regression testing operational
- External service (LLM) somewhat functioning

## Recap of milestone goals

- #4
- Issue #105

## Burnup chart

![alt text](</logs/team/chartW12.png>)

## Table view (Completed)

![alt text](</logs/team/completedW12.png>)

## Table view (In progress)

![alt text](</logs/team/inprogressW12.png>)

## Test report

- Regression testing does pass (In python 3.10, and 3.11)
![alt text](</logs/team/pipelineW12.png>)

- Tests pass in the local virtual environment
![alt text](</logs/team/virtualW12.png>)

## Additional context

- No additional context for now

## Next cycle

- Connecting everything in the project/repository
- Checking environment works after doing so
